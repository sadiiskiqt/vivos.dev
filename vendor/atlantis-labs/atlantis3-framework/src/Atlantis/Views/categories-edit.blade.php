...
@section('title')
Edit Category | A3 Administration | {{ config('atlantis.site_name') }}
@stop
...
<main>
  <section class="greeting">
    <div class="row">
      <div class="columns ">
        <h1 class="huge page-title">Edit Category</h1>
        @if (isset($msgInfo))
  ...
  <section class="editscreen">
    {!! Form::open(['url' => 'admin/categories/edit/' . $oCat->id, 'data-abide' => '', 'novalidate'=> '']) !!}
    <div class="row">
      <div class="columns">
      ...
      <div class="columns small-12">
        <ul class="tabs" data-tabs id="example-tabs">
          <li class="tabs-title is-active main">
            ...
            <a href="#panel1" aria-selected="true">{{ $oCat->category_name }}</a>
            <span class="actions">
              <a data-open="deleteCat" data-tooltip title="Delete Category" class="icon icon-Delete top"></a>
            </span>
          </li>
        </ul>
                ...
                <div class="row">
                  <div class="columns medium-4">
                    ...
                    <label for="category_name" class="is-invalid-label"><span class="form-error is-visible">{{ $errors->get('category_name')[0] }}</span>
                      ...
                      {!! Form::input('text', 'category_name', old('category_name', $oCat->category_name), ['class' => 'is-invalid-input', 'id'=>'category_name']) !!}
                    </label>
                    @else
                    <label for="category_name">Category Name <span class="form-error">is required.</span>
                      <span class="icon icon-Help top" data-tooltip title="Category name can be arbitrary. it let's you organize pages in categories, as well as auto-generate urls."></span>
                      {!! Form::input('text', 'category_name', old('category_name', $oCat->category_name), ['id'=>'category_name', 'required'=>'required']) !!}
                    </label>
                    @endif
                  </div>
                  <div class="columns medium-4">
                    ...
                    <label for="category_string" class="is-invalid-label"><span class="form-error is-visible">{{ $errors->get('category_string')[0] }}</span>
                      ...
                      {!! Form::input('text', 'category_string', old('category_string', $oCat->category_string), ['class' => 'is-invalid-input', 'id'=>'category_string']) !!}
                    </label>
                    @else
                    <label for="category_string">Category String <span class="form-error">is required.</span>
                      <span class="icon icon-Help top" data-tooltip title="Apply this string to the action. This string will be applied when creating a page to the page url , based on the action you have chosen above."></span>
                      {!! Form::input('text', 'category_string', old('category_string', $oCat->category_string), ['id'=>'category_string', 'required'=>'required']) !!}
                    </label>
                    @endif
                  </div>
                  <div class="columns medium-4">
                    <label for="category_action">Category Action
                      ...
                      {!! Form::select('category_action', $actions, $oCat->category_action, ['id' => '']) !!} 
                    </label>
                  </div>
                  <div class="columns medium-4 end">
                    <label for="category_view">Category Default View
                      ...
                      {!! Form::select('category_view', $aTemplates, $oCat->category_view, ['id' => '']) !!} 
                    </label>
                  </div>                  
                </div>
              </div>              
...
<footer>
  ...
  {!!  \Atlantis\Helpers\Modal::set('deleteCat', 'Delete Category', 'Are you sure you want to delete ' . $oCat->category_name, 'Delete', '/admin/categories/delete/' . $oCat->id) !!}
</footer>
@endif
@stop