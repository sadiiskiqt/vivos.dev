...
class MediaWithFilesDataTable implements \Atlantis\Helpers\Interfaces\DataTableInterface {
  ...
  public function getData(\Illuminate\Http\Request $request) {
    ...
    foreach ($modelWithOffset as $k => $obj) {
      ...
      $data[$k] = [
          ...
          'filename' => $this->nameTd($obj, $aGalleries, $filePath, $name),
          'type' => $obj->type,
  ...
  private function nameTd($obj, $aGalleries, $filePath, $name) {
    ...
    if (!empty($obj->thumbnail)) {
      ...
      $dataTag = 'data-image-path="' . $filePath . $obj->thumbnail . '"';
    } else {
      $addToGalleryLink = '';
      $addToGalleryModal = '';
      $dataTag = 'data-file-type="' . $obj->type . '"';
    }

    return '<span class="tags hidden">tags</span>
                    <a class="item" href="/admin/media/media-edit/' . $obj->id . '">' . $name . '</a>
                    <span class="actions">
                    <a class="button alert add-to-gal" ' . $dataTag . ' data-image-id="' . $obj->id . '" id="include-' . $obj->id . '">add<a>
                    </span>' .
