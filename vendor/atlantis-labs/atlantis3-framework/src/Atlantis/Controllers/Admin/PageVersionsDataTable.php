...
class PageVersionsDataTable implements \Atlantis\Helpers\Interfaces\DataTableInterface {
  ...
  public function bulkActions() {
    ...
    return [
        'url' => 'admin/pages/bulk-action-versions',
        'actions' => [
            [
                'name' => 'Delete',
                'key' => 'bulk_delete'
  ...
  public function getData(\Illuminate\Http\Request $request) {
    ...
    $model = DB::table('pages_versions');
    $model->leftJoin('users', 'pages_versions.user_id', '=', 'users.id');
    $model->select('pages_versions.*', 'users.name AS username');   
    ...
    if (isset($request->get('search')['value']) && !empty($request->get('search')['value'])) {
      ...
      $model->where('pages_versions.version_id', 'LIKE', '%' . $search . '%');
      $model->orWhere('pages_versions.updated_at', 'LIKE', '%' . $search . '%');
      $model->orWhere('users.name', 'LIKE', '%' . $search . '%');
    }

    $model->having('pages_versions.page_id', '=', $request->get('page_id'));
    $model->having('pages_versions.language', '=', $request->get('lang'));
    ...
    return response()->json([
                ...
                'recordsTotal' => PagesVersions::where('page_id', '=', $request->get('page_id'))->where('language', '=', $request->get('lang'))->get()->count(),
  ...
  private function nameTd($obj) {
    ...
    if ($obj->active == 0) {
      ...
      return '<span class="tags hidden">tags</span>
                    <a class="item" data-status="' . $status . '" href="/admin/pages/edit/' . $obj->page_id . '/' . $obj->version_id . '/' . $obj->language . '">' . $obj->version_id . '</a>
                    <span class="actions">
                      <a data-tooltip title="Make Active Version" href="/admin/pages/make-active-version/' . $obj->page_id . '/' . $obj->version_id . '/' . $obj->language . '" class="icon icon-Pulse top"></a>
                    ...
                    </span>' . \Atlantis\Helpers\Modal::set('deleteVersion' . $obj->id, 'Delete Version', 'Are you sure you want to delete version ' . $obj->version_id, 'Delete', '/admin/pages/delete-version/' . $obj->page_id . '/' . $obj->version_id . '/' . $obj->language . '');
